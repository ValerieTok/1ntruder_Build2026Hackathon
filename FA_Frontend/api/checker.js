...
export default async function handler(req, res) {
  ...
  const content = [message, context].filter(Boolean).join('\n\n')
  ...
  try {
    ...
    res.status(500).json({ error: getFriendlyBotpressError(error) })
  }
}
...
async function analyzeMessage(message) {
  ...
  const client = await withTimeout(Client.connect({ webhookId, timeout: DEFAULT_TIMEOUT_MS }), DEFAULT_TIMEOUT_MS)
...
async function sendPrompt(client, text) {
  const { conversation } = await withTimeout(client.createConversation({}), DEFAULT_TIMEOUT_MS)
  ...
  return waitForBotReply(client, conversation.id)
}
...
async function waitForBotReply(client, conversationId) {
  ...
  while (Date.now() < deadline) {
    const response = await withTimeout(client.listMessages({ conversationId }), Math.max(1, deadline - Date.now()))
    ...
    if (completedReply) return getMessageText(completedReply)

    if (replies.length) {
      firstReplySeenAt ||= Date.now()
      latestReply = getMessageText(replies[0])
      if (Date.now() - firstReplySeenAt >= 5000) return latestReply
    }
...
function buildAnalysisPrompt(message) {
  return [
    ...
    'riskLevel must be one of Low, Medium, High, or Unclear. Use Unclear only when the supplied message itself lacks enough information.',
...
function isCompleteAnalysis(result) {
  ...
  return riskLevel && riskLevel !== 'unclear' && scamType && scamType !== 'unclear' && Array.isArray(result.redFlags) && result.redFlags.length > 0
}
...
function inferFallbackAnalysis(message, previousReply) {
  ...
  if (/you (?:have )?won|prize|processing fee|upfront fee/i.test(source)) scamType = 'Advance-fee scam'
  else if (/police|government|bank officer|authority|impersonat/i.test(source)) scamType = 'Impersonation scam'
  else if (/password|otp|bank details|account.*(?:locked|suspended)|click (?:the |this )?link/i.test(source)) scamType = 'Phishing scam'
  else if (/job offer|recruiter|task scam|commission/i.test(source)) scamType = 'Job or task scam'
  else if (/investment|crypto|guaranteed return/i.test(source)) scamType = 'Investment scam'

  return {
    riskLevel: redFlags.length >= 2 ? 'High' : redFlags.length === 1 ? 'Medium' : 'Unclear',
    scamType,
    redFlags,
    recommendedAction: 'Do not respond, click links, disclose information, or make payment. Verify the claim through official contact details.',
  }
}

function addRedFlag(redFlags, pattern, source, description) {
  if (pattern.test(source)) redFlags.push(description)
}

function parseBotpressAnalysis(replyText) {
  const trimmed = replyText.trim().replace(/^```(?:json|json5)?\s*/i, '').replace(/\s*```$/, '').trim()
  const candidates = [trimmed]
  const extracted = extractJsonObject(trimmed, false)

  if (extracted && extracted !== trimmed) candidates.push(extracted)
  if (extracted?.startsWith('{{') && extracted.endsWith('}}')) candidates.push(extracted.slice(1, -1))
...
function extractJsonObject(text, required = true) {
  ...
  if (firstBrace === -1 || lastBrace === -1 || lastBrace <= firstBrace) {
    if (required) throw new Error('Botpress did not return structured JSON.')
    return null
  }
...
function looksLikeCompletedAnalysis(text) {
  return Boolean(extractJsonObject(text, false)) || (/risk\s*level\s*:/i.test(text) && /recommended\s*action\s*:/i.test(text))
}
...
function parseLabeledAnalysis(text) {
  ...
  const redFlags = redFlagsBlock
    ? redFlagsBlock.split(/\r?\n|,(?=\s*[A-Z])/).map((flag) => flag.replace(/^[-*\d.)\s]+/, '').trim()).filter(Boolean)
    : []
...
function normalizeAnalysis(result) {
  if (!result || typeof result !== 'object') throw new Error('Botpress returned an invalid result.')
...
function getMessageText(message) {
  return (message.text || message.payload?.text || message.payload?.payload?.text || message.content?.text || message.payload?.markdown || '').trim()
}
...
function getFriendlyBotpressError(error) {
  ...
  if (error.name === 'AbortError' || message.includes('timed out')) return 'Botpress took too long to respond. Please try again.'
  if (message.includes('BOTPRESS_WEBHOOK_ID')) return 'Botpress is not configured yet. Add BOTPRESS_WEBHOOK_ID in Vercel environment variables.'
  if (message.includes('Failed to connect to url')) return 'Botpress could not find the configured webhook. Use the webhook ID from the Botpress Chat Integration webhook URL.'
  if (message.includes('structured JSON') || message.includes('invalid result')) return 'Botpress replied, but not in the expected result format.'
